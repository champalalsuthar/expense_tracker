...
export const action = async ({ request, params }) => {
  ...
  await updateExpense(Number(params.id), formattedData);
  return redirect("/");
};
...
export default function EditExpense() {
  ...
  const [fields, setFields] = useState({
    description: actionData?.values?.description || expense.description,
    amount: actionData?.values?.amount || expense.amount,
    category: actionData?.values?.category || expense.category,
    date: actionData?.values?.date || expense.date.slice(0, 10),
  });
      ...
      >
        ...
        <div className="flex justify-between items-center gap-4">
          <button
            ...
            className={`flex-1 bg-green-600 text-white py-2 px-4 rounded transition ${isSubmitting ? "opacity-50 cursor-not-allowed" : "hover:bg-green-700"
              }`}
          >
